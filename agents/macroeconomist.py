...
from prompts.macroeconomist_prompt import MACROECONOMIST_PROMPT
...
def macroeconomist_node(state: AgentState) -> dict:
    return _run_agent_node(
        state=state,
        prompt_template=MACROECONOMIST_PROMPT,
        output_schema=MacroeconomistOutput,
        agent_name="macroeconomist",
        output_key="macroeconomist_output",
    )
