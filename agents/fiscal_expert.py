...
from prompts.fiscal_expert_prompt import FISCAL_EXPERT_PROMPT
...
class FiscalExpertOutput(BaseModel):
...
def fiscal_expert_node(state: AgentState) -> dict:
    return _run_agent_node(
        state=state,
        prompt_template=FISCAL_EXPERT_PROMPT,
        output_schema=FiscalExpertOutput,
        agent_name="fiscal_expert",
        output_key="fiscal_output",
    )
