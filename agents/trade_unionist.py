...
from prompts.trade_unionist_prompt import TRADE_UNIONIST_PROMPT
...
class TradeUnionistOutput(BaseModel):
...
def trade_unionist_node(state: AgentState) -> dict:
    return _run_agent_node(
        state=state,
        prompt_template=TRADE_UNIONIST_PROMPT,
        output_schema=TradeUnionistOutput,
        agent_name="trade_unionist",
        output_key="trade_unionist_output",
    )
