...
from prompts.institutional_prompt import INSTITUTIONAL_PROMPT
...
def institutional_node(state: AgentState) -> dict:
    return _run_agent_node(
        state=state,
        prompt_template=INSTITUTIONAL_PROMPT,
        output_schema=InstitutionalOutput,
        agent_name="institutional",
        output_key="institutional_output",
    )
